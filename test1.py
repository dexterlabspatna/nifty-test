...
def order(OrderType, Exchange, ExchangeSegment, ScripCode, Quantity, Price, IsIntraday):
    try:
        print("sending order")
        order = Order(order_type=OrderType,exchange=Exchange,exchange_segment=ExchangeSegment, scrip_code = ScripCode, quantity=Quantity, price=Price,is_intraday=IsIntraday)
